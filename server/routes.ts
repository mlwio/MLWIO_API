...
// Alist Configuration
const ALIST_API_URL = process.env.ALIST_API_URL || "https://mlwioalist.up.railway.app";
...
async function getAlistSignedLink(path: string): Promise<string> {
  // Extract path from Alist URLs (https://mlwioalist.up.railway.app/GD01/... -> /GD01/...)
  let alistPath = path;
  if (path.includes('mlwioalist.up.railway.app')) {
    try {
